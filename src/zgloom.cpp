...
#else
SDL_Renderer *ren = nullptr; // Vita: skip SDL renderer; vita2d handles present
#endif

	// Initialize performance hooks (60 FPS limiter)
...
#else
SDL_Texture *rendertex = nullptr; // Vita: not needed when using vita2d presenter
#endif
	...
	while (notdone)
	{
		...
		if (state != STATE_SPOOLING)
{
    // GPU present via vita2d from render32 (software pipeline keeps its scaling)
    ...
    // Overlays intentionally not drawn here to keep title/menu unobstructed.
}
	}
